...
def get_permission(username):
    ...
    try:
        ...
        sql = "select roleId,privilegeId,name,description from bugcount.role_privilege, bugcount.privilege where  role_privilege.privilegeId=privilege.id and roleId=(select roleId from bugcount.user where username = %s)"
        print(f'获取用户权限，sql语句为==', sql)
        print('获取用户权限，sql语句参数 *args==== }', username)
        print('获取用户权限，sql语句参数 *args type==== }', type(username))
        # print(f'获取用户权限，完整的sqlw为 ：select from bugcount.user where username = {username}')
        # userid, username, password, user_remark, user_email, user_level, create_time, session ,roleid
        ...
        for r in sql_return_result_tuple:
            ...
            permission_dict['roleId'] = r[0]
            permission_dict['privilegeId'] = r[1]
            permission_dict['name'] = r[2]
            permission_dict['description'] = r[3]
            ...
            print('????dbutil 转换完的【{}】格式数据users==', permissions)
            """

        # 拼接成【{roid:[数组]} ,name:[数组]】这种格式的，就一个{}
        roleIdArr = []
        privilegeIddArr = []
        nameArr = []
        descriptiondArr = []
        for r in sql_return_result_tuple:
            roleIdArr.append(r[0])
            privilegeIddArr.append(r[1])
            nameArr.append(r[2])
            descriptiondArr.append(r[3])
        permission_dict = dict()
        permission_dict['roleId'] = roleIdArr
        permission_dict['privilegeId'] = privilegeIddArr
        permission_dict['name'] = nameArr
        permission_dict['description'] = descriptiondArr
        permissions.append(permission_dict)
        print('????dbutil 转换完的【{}】格式数据users==', permissions)
        # 拼接返回数据,返回列表
        count = len(sql_return_result_tuple)  # sql语句结果个数
